...
hist(df1$Total.Claim.Amount, main="Histogram for Total Claim Amount", xlab = "Total Claim Auto")
...
df6=select(df5,-c(Effective.To.Date,Customer))
str(df6)

# Converting Data types of some of the variables
df6$Number.of.Open.Complaints = as.factor(df6$Number.of.Open.Complaints)
df6$Number.of.Policies = as.factor(df6$Number.of.Policies)
str(df6)
...
#Iteration 0
train.model1 <- lm(Customer.Lifetime.Value ~ .,data = df6.train)         
summary(train.model1)                                          # R-Squared is 93.43 %
par(mfrow=c(2,2))
plot(train.model1)

#Iteration 1           # Selecting only the Variables with high significance (***)
train.model2 <- lm(Customer.Lifetime.Value ~  I(EmploymentStatus == 'Employed')+ Income+ I(Marital.Status == 'Married')+ Monthly.Premium.Auto+ Number.of.Open.Complaints+ Number.of.Policies,data = df6.train)      
summary(train.model2)                                   # R-Squared is 93.28 % #Adj. R-squared 93.26 %
par(mfrow=c(2,2))
plot(train.model2)

# With the concept of parsimony, we will only select the variables from iteration 1 for our final model.

#### Model Testing  ####

final.model<- lm(Customer.Lifetime.Value ~ I(EmploymentStatus == 'Employed')+ Income+ I(Marital.Status == 'Married')+ Monthly.Premium.Auto+ Number.of.Open.Complaints+ Number.of.Policies,data = df6.test)
...
R.Squared <- 1- SSE/SST
print(R.Squared)        # 92.61 %
mape(results$actual,results$predicted) # 0.099 or 9.9 %
